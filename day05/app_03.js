...
const processAllOperations = () => {
    let result = null;
    let seedsProcessed = 1;

    for (let i = 0; i < arrayOfSeeds.length; i += 2) {
        const startValue = arrayOfSeeds[i];
        const rangeLength = arrayOfSeeds[i + 1];

        for (let j = 0; j < rangeLength; j++) {
            const seedNumber = startValue + j;
            ...
            for (let k = 0; k < numberOfSteps; k++) {
                ...
                if (result > provisionalResult || result === null) {
